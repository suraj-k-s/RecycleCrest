...
//checking if request cotains multipart data
            ...
            //f_item=(FileItem)itr.next();
                        ...
                        if (field_name.equals("txt_photo"))//write the name of image here it is img_product
                        {
                            ...
                            String file_path = proj_path.getParentFile().getParentFile().getPath() + "\\Web\\Assets\\Files\\DriverPhoto\\";//path for storing image
                            Random r = new Random();//to generate random number
                            int r_num = r.nextInt(1111) + 999;//values between this are generated

                            photo = "Driver" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
                        ...
                        if (field_name.equals("txt_proof"))//write the name of image here it is img_product
                        {
                          String ext = file_name.substring(file_name.lastIndexOf("."));//means the extension is stored to variable ext
                           //setting path to store image
                          File proj_path = new File(config.getServletContext().getRealPath("/"));
                          String file_path = proj_path.getParentFile().getParentFile().getPath() + "\\Web\\Assets\\Files\\DriverProof\\";//path for storing image
                          Random r = new Random();//to generate random number
                          int r_num = r.nextInt(1111) + 999;//values between this are generated

                            proof = "DProof" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
                            //creating a file object
           ...
           //to upload proof of doctor
               ...
               // {//Strinh str1="insert into tbl_user ()";//inserting file
                String InsQry = "insert into tbl_driver(driver_name,driver_address,driver_contact,driver_email,driver_gender,driver_dob,location_id,driver_photo,driver_proof,driver_password,driver_doj,company_id)values('" + value[0] + "','" + value[1] + "','" + value[2] + "','" + value[3] + "','" + value[4] + "','" + value[5] + "','" + value[6] + "','"+ photo + "','" + proof + "','" + value[9] + "',curdate(),'"+session.getAttribute("cid")+"')";
   ...
   //System.out.println(InsQry);
        ...
        <script type="text/javascript">
            ...
            setTimeout(function() {
                window.location.href = '../../Company/Driver.jsp'
