...
//checking if request cotains multipart data
            ...
            //f_item=(FileItem)itr.next();
                        ...
                        if (field_name.equals("txt_logo"))//write the name of image here it is img_product
                        {
                            String ext = file_name.substring(file_name.lastIndexOf("."));//means the extension is stored to variable ext
                            //setting path to store image
                            File proj_path = new File(config.getServletContext().getRealPath("/"));
                            String file_path = proj_path.getParentFile().getParentFile().getPath() + "\\Web\\Assets\\Files\\CompanyPhoto\\";//path for storing image
                            Random r = new Random();//to generate random number
                            int r_num = r.nextInt(1111) + 999;//values between this are generated

                            logo = "Company" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
                        ...
                        if (field_name.equals("txt_proof"))//write the name of image here it is img_product
                        {
                          String ext = file_name.substring(file_name.lastIndexOf("."));//means the extension is stored to variable ext
                           //setting path to store image
                          File proj_path = new File(config.getServletContext().getRealPath("/"));
                          String file_path = proj_path.getParentFile().getParentFile().getPath() + "\\Web\\Assets\\Files\\CompanyProof\\";//path for storing image
                          Random r = new Random();//to generate random number
                          int r_num = r.nextInt(1111) + 999;//values between this are generated

                            proof = "CProof" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
                            //creating a file object
           ...
           //to upload proof of doctor
               ...
               // {//Strinh str1="insert into tbl_user ()";//inserting file
                String InsQry = "insert into tbl_company(company_name,company_address,company_contact,company_email,company_logo,company_proof,place_id,company_password,company_doj)values('" + value[0] + "','" + value[1] + "','" + value[2] + "','" + value[3] + "','" + logo + "','" + proof + "','" + value[5] + "','" + value[6] + "',curdate())";
